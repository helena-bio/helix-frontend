    ...
    <div className="bg-card border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="space-y-1">
          ...
          {c.conditions.map((cond, i) => (
            <div key={i} className="flex items-start gap-2">
              ...
              <p className="text-sm text-muted-foreground">{cond}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {c.exclusions && c.exclusions.length > 0 && (
        <div className="space-y-1">
          ...
          {c.exclusions.map((exc, i) => (
            <p key={i} className="text-sm text-muted-foreground">{exc}</p>
          ))}
        </div>
      )}
      ...
      {(c.databases || c.limitations || c.note) && (
        <div className="pt-2 border-t border-border space-y-1">
          {c.databases && <p className="text-sm text-muted-foreground"><span className="font-medium text-foreground">Databases: </span>{c.databases}</p>}
          {c.limitations && c.limitations.map((lim, i) => <p key={i} className="text-sm text-muted-foreground">{lim}</p>)}
          {c.note && <p className="text-sm text-primary/80">{c.note}</p>}
        </div>
      )}
    </div>
  )
